...
export function buildingIdStoreReducer(state, action) {
  const appStateId = action.payload.componentId;
  return setConfigInApplicationState(appStateId, state, ['buildingIdRecord'], action.payload.obj.id);
}

export function buildingIdRecordAction(id, obj) {
    return {
      type: id + '.BUILDING_ID_RECORD_STORE',
      payload: {
        componentId: id,
        obj: obj
      }
